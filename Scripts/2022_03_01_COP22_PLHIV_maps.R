...
# MUNGE ============================================================================
  ...
  sds_maps <- 
    ...
    mutate(zmb_plhiv = sum(tot_plhiv, na.rm = T),
...
# PLHIV TOTAL -------------------------------------------------------------
 ...
 plhiv_tot <- get_map(sds_maps, metric = tot_plhiv) +
   scale_fill_viridis_c(alpha = 0.75, trans = "log", option = "B", 
                        direction = -1,
                        labels = comma, 
                        breaks = 10^(3:6))
 ...
 plhiv_hist <- get_hist(sds_maps, metric = tot_plhiv) +
   ...
   scale_fill_viridis_c(alpha = 0.75, option = "B", 
                        direction = -1,
                        labels = comma, 
                        guide = "none") 
...
# ART COVERAGE ------------------------------------------------------------
 ...
 zmb_cov_ave <- max(sds_maps$zmb_coverage)
  ...
  cov_map <-  get_map(sds_maps, metric = coverage) +
   scale_fill_viridis_c(alpha = 0.75, option = "A", direction = -1, labels = percent, 
  ...
  cov_hist <-  get_hist(sds_maps, metric = coverage, lines = 13, bins = 40) +
    scale_x_continuous(labels = percent(seq(0, 1.3, 0.25), 1), breaks = seq(0, 1.3, 0.25),
                       limit=c(0, 1.3)) +
...
# VIRAL LOAD COVERAGE -----------------------------------------------------
 ...
 # How many NA values
 sds_maps %>% filter(is.na(VLC)) %>% count()
 ...
 vlc_map <- get_map(sds_maps, metric = VLC) +
   scale_fill_viridis_c(alpha = 0.75, option = "C", direction = -1, labels = percent, 
                        na.value = grey20k, guide = "none",
 ...
 vlc_hist <- get_hist(sds_maps, metric = VLC, bins = 40) +
   scale_x_continuous(labels = percent(seq(0, 1, 0.1), 1), breaks = seq(0, 1, 0.1),
                       limits = c(vlc_min, vlc_max)) +
