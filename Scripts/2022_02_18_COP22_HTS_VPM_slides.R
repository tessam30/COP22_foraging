...
df_hts_viz %>%
  ...
  geom_label(
    data = . %>% filter(period %in% c("FY20Q4", "FY21Q4", "FY22Q1")),
  ...
  labs(
    title = "HTS_TST_POS QUARTERLY ACHIEVEMENT",
...
top <- df_hts_sex %>%
  ...
  labs(
    title = "HTS_TST_POS VOLUME BY SEX",
...
bottom <- df_hts_mod_agg %>%
  ...
  labs(
    title = "PEPFAR HTS_TST_POS MODALITY MIX ACROSS OVER TIME",
...
b <- df_hts_combo_snu1 %>%
  ...
  labs(
    y = NULL, x = NULL, title = "HTS_TST_POS FY22Q1 RESULTS TO TARGETS",
    caption = data_source
  )
...
df_bars %>%
  ...
  { # If there are no disags that have achieved targets, then ignore the dividing line
    if (Switch) {
      geom_col(
        data = . %>% filter(hline_var == 1),
        aes(y = snu1_order, x = Inf), width = 0.05,
        position = position_nudge(y = -0.55), fill = grey30k
      )
    }
  } +
  facet_wrap(sex ~ ageasentered,
    scales = "free",
...
df_index <-
  df %>%
  filter(
    indicator %in% c("HTS_INDEX_NEWPOS", "HTS_TST_POS"),
    standardizeddisaggregate == "Total Numerator"
  )
...
df_index %>%
  ...
  geom_text(aes(y = pct_pos, label = percent(pct_pos, 1)),
...
nnt_calc <- function(df, ...) {
  df %>%
    ...
    spread(indicator, results) %>%
    mutate(
      NNT = ifelse(HTS_TST_POS > 0, HTS_TST / HTS_TST_POS, NA_real_),
...
df_viz %>%
  ...
  mutate(
    name = fct_relevel(name, c("NNT", "HTS Positivity", "HTS_TST", "HTS_TST_POS")),
    text_lab = case_when(
  ...
  geom_col(
    data = . %>% filter(ageasentered == "15-19"),
    aes(x = value),
    fill = "#afc9f0",
    width = 0.25,
...
df_hts_nnt %>%
  ...
  geom_text(aes(x = NNT, label = comma(NNT, 1)),
    hjust = -0.2, size = 9 / .pt,
    family = "Source Sans Pro"
  ) +
  scale_color_si(palette = "scooters") +
  ggnewscale::new_scale_color() +
  geom_text(aes(x = neg_offset, label = percent(positivity, 1), color = ifelse(positivity > .10, "white", "black")),
    size = 9 / .pt,
    family = "Source Sans Pro"
  ) +
  scale_color_identity() +
...
df_hts_agyw_viz %>%
  ...
  filter(
    indicator == "HTS_TST_POS", trendssemifine == "20-24",
    snu1 != "PEPFAR"
  ) %>%
  ggplot(aes(x = period)) +
  geom_col(aes(y = targets), fill = trolley_grey_light) +
...
mutate(mod_type = case_when(
  ...
  ungroup() %>%
  mutate(
    start = case_when(period == min(period) ~ contribution),
...
df %>%
  filter(
    indicator %in% c("HTS_TST", "HTS_TST_POS", "TX_CURR", "TX_NEW", "TX_NET_NEW", "TX_PVLS"),
    ageasentered %in% c("15-19", "20-24")
  ) %>%
...
df_cascade %>%
  ...
  si_style_ygrid(text_scale = 1.25) +
  labs(
    x = NULL, y = NULL, title = "ZAMBIA CASCASE - FY22 Q1 RESULTS TO TARGETS (GRAY BARS)",
...
df_mech <-
  ...
  mutate(
    achv = ifelse(targets > 0, cumulative / targets, NA_real_),
    gap = ifelse(targets > 0, targets - cumulative, NA_integer_),
    snu1 = str_remove_all(snu1, " Province")
  ) %>%
  group_by(snu1, sex, psnu) %>%
  mutate(
    total_gap = sum(gap, na.rm = T),
...
indic <- df_achv_htspos %>%
  distinct(indicator) %>%
  pull()

df_achv_htspos %>%
  filter(
    facet_label == "Unachieved",
    achv > 0, targets > 200
  ) %>%
  ...
  labs(
    x = NULL, y = NULL, title = glue("AGENCY SHORTFALLS BY PSNU - {indic}"),
...
df %>%
  mutate(
    ymax = ifelse(indicator == "HTS_TST", 2.9e6, 2.25e5),
    ymin = 0
  ) %>%
  mutate(facet_order = fct_relevel(
    indicator, "HTS_TST",
    "HTS_TST_POS",
    "HTS_SELF"
  )) %>%
  ggplot(aes(x = period)) +
  geom_col(aes(y = targets),
    fill = grey30k,
